...
(         MONTH(a.TanggalMulai)=?
    ...
    public function read_pekerjaan_pengguna_by_pengguna_tahun_bulan_2($dataInput)
    {
...
(         MONTH(a.TanggalMulai)=?
      ...
      $query_tambahan3
       ;
        ...
        foreach ($query->result_array() as $row) {
            ...
            $masukan = [
                "Bulan" => $dataInput['Bulan'],
                "Tahun" => $dataInput['Tahun'],
                "IdDinilai" => $dataInput['PenerimaPekerjaanId']

            ];
            $row['PenilaianKepala'] = $this->model_penilaian_tim->read_nilai_dari_kepala($masukan);;
            $row['SelisihHari'] = (new DateTime($row['TanggalSelesai']))->diff(new DateTime($row['TanggalMulai']))->days + 1;
    ...
    public function ubah_volume_realisasi_pekerjaan_pengguna_by_id($dataInput)
    {
        // print_r($dataInput);
        $this->db->query("update pekerjaan_bulanan_pengguna 
